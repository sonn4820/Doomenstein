...
void Map::StartUp()
{
	...
	m_vertexBuffer = g_theRenderer->CreateVertexBuffer(sizeof(Vertex_PCUTBN) * m_vertexes.size());
...
void Map::SpawnPlayer(int playerID)
{
	...
	for (;;)
	{
		if (playerID == 1)
		{
			randomSpawnPointIndex = g_theRNG->RollRandomIntLessThan((int)spawnPointIndexes.size(), true);
